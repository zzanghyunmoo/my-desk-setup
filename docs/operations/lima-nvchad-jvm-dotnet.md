...
`launchSettings.json` profile을 사용한다.
장기 실행 작업은 `:MdsProjectCancel`로 process group까지 종료한다.
...
listen readiness를 확인한 뒤 해당 adapter를 자동 attach한다. .NET application debugging은 선택한 project의
`TargetPath`, cwd, environment와 loopback URL을 그대로 DAP configuration에
반영한다. `debug-test`는 `VSTEST_HOST_DEBUG=1`로 시작한 testhost PID에
NetCoreDbg를 attach한다.
