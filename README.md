...
ZZA-100 기능 브랜치에서 `plan`, `apply`, `doctor`, `update`와 release 및
certification 도구를 구현하고 검증하는 중이다. 아래 항목은 아직 완료로
간주하지 않는다.

- Windows host, WSL Ubuntu와 Lima Ubuntu의 실제 target evidence는 아직 없다.
- fixture와 CI smoke는 실제 머신 evidence를 대신하지 않는다.
- 기존 `settings` 원격의 rename, orphan history, force-push와 workspace
  submodule 전환은 별도 파괴적 승인 전까지 실행하지 않는다.
...
digest와 target preimage가 달라지면 첫 mutation 전에 중단한다.
...
준비한 뒤에는 같은 release의 Linux `mds`를 게스트 안에서 실행한다.
...
[bootstrap 운영 문서](docs/operations/bootstrap.md)에 있다.
